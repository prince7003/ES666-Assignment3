...
class PanaromaStitcher:
    ...
    def compute_homography(self, pts1, pts2):
        max_iterations = 2000
        threshold = 5.0  # Adjusted threshold for better inlier selection
        best_H = None
        max_inliers = 0
        best_inliers = []
    ...
    def Homography_to_points(self, H, pts):
        pts_homogeneous = np.hstack([pts, np.ones((pts.shape[0], 1))])
    ...
    def inverse_warp(self, img1, img2, H):
        ...
        transformed_corners = self.Homography_to_points(H, corners_img2)
