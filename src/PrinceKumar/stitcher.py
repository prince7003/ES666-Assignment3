...
class PanaromaStitcher():
    ...
    def dlt(self, p1, p2):
        p1_norm, T1 = self.normalize_points(p1)
        p2_norm, T2 = self.normalize_points(p2)
        A = []
        for i in range(len(p1_norm)):
            x, y = p1_norm[i]
            x_prime, y_prime = p2_norm[i]
    ...
    def compute_homography(self, p1, p2):
        ...
        for _ in range(max_iterations):
            ...
            projected_p2_homogeneous = (H_candidate @ p1_homogeneous.T).T

            projected_p2_homogeneous[projected_p2_homogeneous[:, 2] == 0, 2] = 1e-10
            projected_p2 = projected_p2_homogeneous[:, :2] / projected_p2_homogeneous[:, 2, np.newaxis]

            errors = np.linalg.norm(p2 - projected_p2, axis=1)
        ...
        if best_H is not None and len(best_inliers) >= 10:  #atleast 10 inliers
            best_H = self.dlt(p1[best_inliers], p2[best_inliers])
        else:
            return None

        return best_H

    def apply_homography_to_points(self, H, p):
        p_homogeneous = np.hstack([p, np.ones((p.shape[0], 1))])
        transformed_p = (H @ p_homogeneous.T).T
        transformed_p[transformed_p[:, 2] == 0, 2] = 1e-10
        transformed_p = transformed_p[:, :2] / transformed_p[:, 2, np.newaxis]
    ...
    def make_panaroma_for_images_in(self,path):
        ...
        for i in range(1, len(Img_list)):
            ...
            H = self.compute_homography(p2, p1)
            if H is None:
                continue

            homography_matrix_list.append(H)
            stitched_image = self.inverse_warp(stitched_image, Img_list[i], H)
